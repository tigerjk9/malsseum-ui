...
- **AccessGate is shown on every mount, even when credentials are saved.** With saved creds, the gate opens to a `welcome` step (hero + mode badge + "들어가기" button) — no re-entry of password/API key required, just one click to enter the chat. Without creds, it opens to `select` as before. When the user opens the gate manually (TopBar LockIcon or admin "접속 전환"), `gateForceSelect=true` skips welcome and lands on `select` so they can actually change modes. Don't collapse welcome and select — welcome's job is hero exposure + frictionless entry, select's job is mode change.
